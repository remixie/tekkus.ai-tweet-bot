...
export class AIService {
  ...
  constructor() {
    ...
    this.jsonFilePath = path.join(process.cwd(), 'twitter-UserTweets-1752162517013.json');
  }

  async generateResponse(userMessage: string, tweets?: Tweet[]): Promise<string> {
    try {
  ...
  private buildSmartContext(userMessage: string, tweets: Tweet[]): string {
    ...
    const dateRange = tweets.length > 0 ? 
    ...
    const searchDebugInfo = searchTerms.length > 0 ? `\nSearch terms used: ${searchTerms.join(', ')}` : '';
  ...
  private searchAllTweets(userMessage: string, allTweets: Tweet[]): Tweet[] {
    ...
    const topRelevantTweets = searchResults.slice(0, 150).map(result => result.tweet);
    
    // Combine recent tweets and relevant tweets, removing duplicates
    const combinedTweets = this.deduplicateTweets([...recentTweets, ...topRelevantTweets]);
  ...
  private grepSearchJsonFile(searchTerms: string[]): Set<string> {
    ...
    try {
      ...
      console.log(`Node.js grep found ${foundTweetIds.size} matching tweet IDs for terms: ${searchTerms.join(', ')}`);
  ...
  private extractSearchTerms(userMessage: string): string[] {
    ...
    const stopWords = new Set(['the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'is', 'are', 'was', 'were', 'be', 'been', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should', 'may', 'might', 'can', 'about', 'what', 'when', 'where', 'why', 'how', 'who', 'which', 'that', 'this', 'these', 'those', 'i', 'you', 'he', 'she', 'it', 'we', 'they', 'me', 'him', 'her', 'us', 'them']);
    
    // Extract words, quoted phrases, and meaningful terms
    const words = message.match(/\b\w+\b/g) || [];
    const quotedPhrases = message.match(/"([^"]+)"/g)?.map(phrase => phrase.slice(1, -1)) || [];
    
    // Add semantic expansion for common queries
    const expandedTerms: string[] = [];
    
    // If asking about release/launch dates, add related terms
    if (message.includes('release') || message.includes('launch') || message.includes('drop') || message.includes('when')) {
      expandedTerms.push('introducing', 'coming', 'going live', 'launch', 'release', 'drop', 'available', 'announcing', 'debut');
    }
  ...
  private calculateRelevanceScore(tweet: Tweet, searchTerms: string[], originalMessage: string): number {
    const tweetText = tweet.text.toLowerCase();
    let score = 0;
    
    // Check for exact phrase matches (highest score)
    const quotedPhrases = originalMessage.match(/"([^"]+)"/g)?.map(phrase => phrase.slice(1, -1).toLowerCase()) || [];
    ...
    const daysSincePosted = (Date.now() - new Date(tweet.timestamp).getTime()) / (1000 * 60 * 60 * 24);
...
Your role is to be an accurate, data-driven representation of @tekkusai based on their actual Twitter activity.`;
    ...
    try {
      ...
      this.tweets = this.jsonData
        .filter(tweet => {
          ...
          if (tweet.full_text.startsWith('RT @KurosunCo:') || 
      ...
      this.isLoaded = true;
      console.log(`Successfully loaded ${this.tweets.length} tweets from JSON file`);
      
      if (this.tweets.length > 0) {
        const oldestTweet = this.tweets[this.tweets.length - 1];
        const newestTweet = this.tweets[0];
        console.log(`Tweet range: ${new Date(newestTweet.timestamp).toLocaleDateString()} to ${new Date(oldestTweet.timestamp).toLocaleDateString()}`);
      }
  ...
  async getTweetStats(): Promise<{
    ...
    const totalLength = this.tweets.reduce((sum, tweet) => sum + tweet.text.length, 0);
